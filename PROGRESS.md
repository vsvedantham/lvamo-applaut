...
| Database | Live | PostgreSQL 16 in Docker on `lvamo-backend`, DB/role named `lvamo` (renamed from `applaut` Aug 2026 — see below) — one Postgres database, one alembic migration chain, shared by both verticals but namespaced at the schema level: Applaut's 10 tables live in an `applaut` Postgres schema (migration `0012`, deployed to prod, see `PROGRESS_APPLAUT.md`). Jobref went further: its 3 tables were merged into a single `jobref.users` table in the `jobref` schema (migration `0013`, `is_employee` boolean as the sole differentiator, deployed to prod — see `PROGRESS_JOBREF.md`). |
