...
**Jobref deployed (Aug 2026, two rounds same day)**: first round shipped LinkedIn-gated registration (migrations `0007`+`0008`); a same-day follow-up split registration so LinkedIn is job-seeker-only — employees register directly (user's request), plus an app-wide mobile CSS fix (input font-size/tap-target sizing, affects Applaut too — see `PROGRESS_APPLAUT.md`) — migration `0009`. Both rounds pushed, migrated, and redeployed via `deploy.sh` + Cloudflare Pages. Verified in production with real data: a genuine employee account created end-to-end via the live API (register → 201, duplicate email → 409, login → 200), LinkedIn authorize redirect correct, Applaut regression clean each time. **One gap remains**: nobody has completed a real job-seeker registration through the actual LinkedIn consent screen + final submission — only the OAuth handshake itself and a mocked-token submission have exercised that path. See `PROGRESS_JOBREF.md` for detail.
