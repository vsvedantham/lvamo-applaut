...
| **Jobref** | Live — auth, companies, and the full referral request flow (routing, employee inbox, seeker limits) deployed Aug 2026 | [`PROGRESS_JOBREF.md`](PROGRESS_JOBREF.md) — primary task: employee-side actions on a request (accept/decline) |
...
| Database | Live | PostgreSQL 16 in Docker on `lvamo-backend`, DB/role named `lvamo` (renamed from `applaut` Aug 2026 — see below) — one Postgres database, one alembic migration chain (head: `0017`), shared by both verticals but namespaced at the schema level: Applaut's 10 tables live in an `applaut` Postgres schema (migration `0012`, deployed to prod, see `PROGRESS_APPLAUT.md`). Jobref went further: its 3 tables were merged into a single `jobref.users` table (migration `0013`), plus `jobref.companies` (migration `0014`) and `jobref.referral_requests` (migrations `0015`–`0017`) — the full referral request/routing/limits feature set, deployed to prod, see `PROGRESS_JOBREF.md`. |
