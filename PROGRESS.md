...
**Jobref deployed (Aug 2026, four rounds same day)**: LinkedIn-gated registration (migrations `0007`+`0008`) → split registration so LinkedIn is job-seeker-only, employees register directly, plus an app-wide mobile CSS fix affecting Applaut too (migration `0009`, see `PROGRESS_APPLAUT.md`) → progressive-disclosure registration UX (frontend-only) → new required employee field ("daily referral-view cap") + reworded seeker disclaimer (migration `0010`). All four pushed and redeployed via `deploy.sh` + Cloudflare Pages, each verified in production with real data: a genuine employee account created end-to-end via the live API (register → 201, duplicate email → 409, login → 200, missing-required-field → 422), LinkedIn authorize redirect correct, Applaut regression clean each round, frontend re-verified live against `www.lvamo.com` itself each time. **One gap remains**: nobody has completed a real job-seeker registration through the actual LinkedIn consent screen + final submission — only the OAuth handshake itself and a mocked-token submission have exercised that path. See `PROGRESS_JOBREF.md` for detail.
