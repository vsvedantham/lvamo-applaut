...
| Database | Live | PostgreSQL 16 in Docker on `lvamo-backend` — one Postgres database, one alembic migration chain, shared by both verticals but namespaced at the schema level: Applaut's 10 tables live in an `applaut` Postgres schema (migration `0012`, see `PROGRESS_APPLAUT.md`; **not yet deployed to prod**, verified locally only). Jobref went further: its 3 tables were merged into a single `jobref.users` table (migration `0013`, `is_employee` boolean as the sole differentiator — see `PROGRESS_JOBREF.md`); **also not yet deployed**, prod still runs the old `public.jobref_users`/`jobref_employee_profiles`/`jobref_seeker_profiles` split (migrations `0007`+`0008` applied in prod Aug 2026). |
